...
- **即开即写** — 浏览器内直接编写 Python 代码，自带完整编辑器，无需配置本地开发环境
- **边写边验** — 提交后秒出评测结果，每个测试点的通过情况与错误细节一目了然
- **手写后再对照** — 完成自己的实现后，解锁带逐行讲解的标准答案，深入理解每一行背后的设计思路
- **理论题库（八股文）** — 13 章结构化面试题，覆盖大模型架构、对齐技术、分布式训练、推理优化等核心话题
- **模拟考场** — 随机抽取编码题与理论题组成完整面试流程，提前适应真实面试的紧张节奏
- **按图索骥** — 从注意力机制到图神经网络，跟着系统化的学习路线循序渐进
- **断点续练** — 做题记录、尝试次数、完成度自动持久化，刷新页面或换设备也不归零
- **智能提示** — 按需接入任意 OpenAI 兼容接口，获得个性化的代码提示与解题思路引导
- **本地闭环** — 评测全链路跑在本地，代码与数据不离开你的电脑（AI 提示功能除外）
