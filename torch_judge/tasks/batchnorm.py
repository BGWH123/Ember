...
def my_batch_norm(
    ...
    """
    if training:
        batch_mean = x.mean(dim=0)  # 求均值
        batch_var = x.var(dim=0, unbiased=False)  # 求方差
    ...
    x_norm = (x - mean) / torch.sqrt(var + eps)  # 开方运算
