...
TASK = {
    ...
    "hint_zh": (
        "MHA：`nn.Linear(d, 3d)` → 拆分 Q/K/V → reshape `(B, H, N, d_h)` → 缩放点积 → `proj`\nGELU：`x * 0.5 * (1 + erf(x / sqrt(2)))`\n块结构：`x = x + MHA(norm1(x))` → `x = x + MLP(norm2(x))`"
    ),
    "tests": [
        {
            "name": "Output shape",
