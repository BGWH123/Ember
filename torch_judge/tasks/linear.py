...
TASK = {
    ...
    "hint_zh": (
        "`weight` 形状 `(out, in)`，初始化 `randn * 1/sqrt(in_features)`\n`bias` 形状 `(out,)`，初始化为零\n前向：`x @ weight.T + bias`"
    ),
    "tests": [
        {
...
class SimpleLinear:
    """
    Simple Linear Layer module.
    """
    def __init__(self, in_features: int, out_features: int):  # 初始化: 定义模型结构和参数
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:  # 前向传播: 定义数据流
