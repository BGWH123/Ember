...
TASK = {
    ...
    "hint_zh": (
        "1. 稳定 softmax：`exp(logits - max)` → 归一化 → `probs`\n2. `p_t = probs[arange(N), targets]`\n3. `return (-alpha * (1-p_t)**gamma * log(p_t+1e-8)).mean()`"
    ),
    "tests": [
        {
            "name": "Returns scalar",
