...
TASK = {
    ...
    "hint_zh": (
        "将 Q/K/V 分成 num_devices 块。对每个 Q 块 i，遍历所有 K/V 块（环形顺序）。\n每步 online softmax：\n  m_new = max(m, scores.max)\n  l_new = l·exp(m-m_new) + Σexp(scores-m_new)\n  o = (o·l·exp(m-m_new) + exp(scores-m_new)@V) / l_new"
    ),
    "tests": [
        {
            "name": "Output shape",
...
def ring_attention(Q, K, V, num_devices):
    ...
    for i, Qi in enumerate(Q_chunks):
        ...
        for j in range(num_devices):
            ...
            m_new = torch.maximum(m, scores.max(dim=-1, keepdim=True).values)  # 求最大值
            exp_scores = torch.exp(scores - m_new)  # 指数运算
            l_new = l * torch.exp(m - m_new) + exp_scores.sum(dim=-1, keepdim=True)  # 指数运算
            o = o * (l * torch.exp(m - m_new)) / l_new + (exp_scores @ Vj) / l_new  # 指数运算
            m, l = m_new, l_new
