...
def flash_attention(Q, K, V, block_size=32):
    ...
    for i in range(0, S, block_size):
        ...
        for j in range(0, S, block_size):
            kj = K[:, j:j+block_size]
            vj = V[:, j:j+block_size]
            scores = torch.bmm(qi, kj.transpose(1, 2)) / math.sqrt(D)  # Transpose to (B, heads, S, d_k)
            block_max = scores.max(dim=-1, keepdim=True).values  # 求最大值
            new_max = torch.maximum(row_max, block_max)
            correction = torch.exp(row_max - new_max)  # 指数运算
            exp_scores = torch.exp(scores - new_max)  # 指数运算
            acc = acc * correction + torch.bmm(exp_scores, vj)  # 批量矩阵乘法
            row_sum = row_sum * correction + exp_scores.sum(dim=-1, keepdim=True)  # 求和
            row_max = new_max
